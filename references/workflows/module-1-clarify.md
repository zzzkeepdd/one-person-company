...
### Phase 2.3: 维度聚合（v3.1 新增）

Hermes 统筹收集所有维度 JSON → 聚合为完整辩论输出：
1. 合并所有 verified_items（去重）
2. 合并所有 must_fix（去重）
3. 检查跨维度冲突（如策略要求高频但风控要求间隔5秒）→ 写入 must_fix，标注 cross_dimension_conflicts
4. **关键：子Agent产出的 JSON 字段名各异，Hermes必须标准化为验证器期等格式**：
   - `rounds_completed`（非 debate_rounds_completed）
   - `dimensions_covered` 为 `[{name, status}]` 列表（非 dict）
   - `attacks` 为平铺列表（非 `{维度名: [攻击数组]}` 嵌套 dict）
   - `converged` 为 boolean
   - `verified_items` 为非空数组
5. 产出完整 debate-output JSON
